...
export const addTransaction = async (req: Request, res: Response) => {
  try {
    ...
    console.error('ADD TRANSACTION ARROR', err);
...
export const updateTransaction = async (req: Request, res: Response) => {
  try {
    ...
    const findTransaction = await prisma.expense.findUnique({
      where: {
...
export const deleteTransaction = async (req: Request, res: Response) => {
  try {
    const { id: idTransaction } = req.params;
    const beloningTransaction = await prisma.expense.findUnique({
      where: {
        id: Number(idTransaction),
      },
    });
    if (beloningTransaction?.userId !== req.user?.id) {
      return res
        .status(403)
        .json({ message: 'This transaction isn"t belong you' });
    }
